#!/usr/bin/env bash
# Exact native D/C rejection differential for §17.3 channel-operation misuse.
# These rows reject before the compiler can produce a binary, so they are kept
# beside (not inside) the executable interpreter differential.

set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CORPUS_DIR="${CHANNEL_REJECT_CORPUS_DIR:-$REPO_ROOT/selfhost/channel_reject_corpus}"
MANIFEST="${CHANNEL_REJECT_MANIFEST:-$REPO_ROOT/selfhost/channel_reject_corpus.txt}"
...
fail_setup() {
  echo "channel-reject-diff SETUP: $*"
  exit 2
}
...
if ! cmp -s "$tmp/all" "$tmp/manifest"; then
  echo "channel-reject-diff SETUP: corpus is not exactly enumerated by manifest"
...
while IFS= read -r name; do
  ...
  if [ "$interp_rc" -eq "$compiler_rc" ] \
    ...
    echo "PASS $name (channel reject rc=$interp_rc)"
    pass=$((pass + 1))
  else
    echo "FAIL $name: D/C channel rejection differs (interpreter rc=$interp_rc, compiler rc=$compiler_rc, expected rc=$expected_rc)"
...
echo "-- canonical channel rejection differential: $pass exact, $fail failed --"
[ "$fail" -eq 0 ]
