#!/usr/bin/env bash
# Exact native D/C rejection differential for non-concurrent §6.3 loop errors.
# These rows cannot enter interpreter_diff.sh because the compiler must reject
# before it can produce a binary.

set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CORPUS_DIR="${CONTROL_REJECT_CORPUS_DIR:-$REPO_ROOT/selfhost/control_reject_corpus}"
MANIFEST="${CONTROL_REJECT_MANIFEST:-$REPO_ROOT/selfhost/control_reject_corpus.txt}"
...
fail_setup() {
  echo "control-reject-diff SETUP: $*"
  exit 2
}
...
if ! cmp -s "$tmp/all" "$tmp/manifest"; then
  echo "control-reject-diff SETUP: corpus is not exactly enumerated by manifest"
...
while IFS= read -r name; do
  ...
  if [ "$interp_rc" -eq "$compiler_rc" ] \
    ...
    echo "PASS $name (control reject rc=$interp_rc)"
    pass=$((pass + 1))
  else
    echo "FAIL $name: D/C control rejection differs (interpreter rc=$interp_rc, compiler rc=$compiler_rc, expected rc=$expected_rc)"
...
echo "-- canonical control rejection differential: $pass exact, $fail failed --"
[ "$fail" -eq 0 ]
